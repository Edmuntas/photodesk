...
## [2026-04-04] — CRM Лакохот + Верстка AI Cleaner + Удаление сессий

### Сделано

**CRM — Лакохот (photodesk.html)**
- `renderAgentsView()` card mode: новый класс `.agent-card` с вертикальным layout, обёртка `#customers-list:has(.agent-card)` через CSS grid — агенты теперь отображаются сеткой, а не строками
- Кнопка "🔀 מיזוג" добавлена в шапку панели клиента (`.cust-quick-actions`) — видна без входа в Edit-таб
- Форма "פרטים" расширена: добавлены поля **טלפון נוסף** (второй телефон) и **מספר רישיון** (лицензия агента)
- `migrateToCompanies()`: сброс невалидного `companyId` — импортированные клиенты с несуществующей компанией теперь автоматически переназначаются в CO_PRIVATE или нужный Мисрад

**AI Cleaner верстка (declutter.html)**
- `.preview-wrap`: добавлен `min-height:0` — критично для flex-chain, предотвращает раздувание viewer-панели
- `.comparison`: убран `max-height:calc(100%-60px)` → `flex:1;min-height:0`
- `#img-main`: убран `max-height:calc(100%-110px)` → `max-height:100%;flex:1;min-height:0`
- `.split-view`: убран `max-height:calc(100%-50px)` — ограничивается flex-родителем
- Мобильный layout: `.panel-right` получил `max-height` для жёсткого ограничения высоты

**Удаление сессий (declutter.html + server.js)**
- Backend: новый endpoint `DELETE /api/sessions/:slug` — удаляет папку сессии рекурсивно через `fs.rmSync`
- Frontend: кнопка 🗑 в каждой session-карточке (появляется при hover), confirm-диалог, удаление карточки из DOM
- CSS `.sc-delete`: кнопка скрыта по умолчанию, появляется при наведении на карточку

### Исправлено
- Сервер перезапущен вручную — новый DELETE endpoint не подхватывался без перезапуска Node.js процесса

### Известные проблемы / TODO
- Сервер не перезапускается автоматически при изменении файлов (nodemon не настроен)
- Sky mode и lighting не тестировались с реальным Aurora API в этой сессии
